...
const RequestDetails = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const request = await getStockRequestById(id);
                setRequest(request);
                setModifiedItems(request.items); // Initialize modified items
                const historyData = await getStockRequestHistory(id);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await updateStockRequest(id, updatedRequest);
    ...
    return (
        <div className='container mx-auto bg-card p-4 h-screen flex flex-col gap-4'>
            <header className=" p-2 flex justify-between items-center">
                <Link to="/" className="flex items-center">
                    <Zap size={32} strokeWidth={1} className="text-amber-500" />
                    <h1 className="text-2xl font-medium">
                        Insight<span className="text-amber-500 text-2xl ml-1">X</span>
                    </h1>
                </Link>
            </header>
            <div className="h-full flex flex-col justify-center items-center">
            <div className="sm:max-w-5xl w-full  mx-auto bg-white p-5 rounded-md shadow-sm">
                <div className="space-y-2">
                    <h1 className='text-4xl'>{request?.request_title}</h1>
                ...
                {loading ? (
                    ...
                    <div className="space-y-4 my-4">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">
                            Requested by <span className="font-medium text-blue-500">{request.requested_by}</span> on <span className="font-medium text-blue-500"> {moment(request.created_at).format('MMMM DD, YYYY, h:mm:ss a')} </span>
                        </p>
