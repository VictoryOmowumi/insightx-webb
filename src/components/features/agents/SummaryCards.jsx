...
import { agentsSummary } from "@/services/api";
...
const SummaryCards = () => {
  ...
  useEffect(() => {
    const fetchSummaryData = async () => {
      try {
        const response = await agentsSummary();
      ...
      {loading
        ...
        : summaryData.map((kpi, index) => (
            <div key={index} className="flex flex-col gap-[1px]">
              <div className="flex gap-1">
                <div className="flex mt-5 items-center justify-center w-6 h-6 rounded-full bg-[#B6B6B6]/20">
                  {kpi.label === "Total Agents" && <Activity size={10} strokeWidth={1} />}
                  {kpi.label === "Active Agents" && <List size={12} strokeWidth={1} />}
                  {kpi.label === "Agents with Submissions" && <CheckCircle size={12} strokeWidth={1} />}
                  {kpi.label === "Agents without Submissions" && <Calendar size={12} strokeWidth={1} />}
                </div>
